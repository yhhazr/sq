...
function regHandler()
{
	...
	$.ajax({
		   ...
		   success:function(data) {
			   if(data.result == 'false') {
					$('#reg_tip').html(data.respMsg);
					$("#reg_uid").val("");
				} else if(data.result == 'true') {
					$('body').append(data.bbsData);
					var canLogGame = enterGame(LOGINGAME + newGamezoneId);
					if(canLogGame == 0) {
						setTimeout(function(){location = LOGINGAME + newGamezoneId + '&timestamp=' + Date.parse(new Date());	},500);
						return true;
					}else {
	...
	if(!regLetterFirst.test(uid.val())) {
		...
		$.get(url,function(data){
			if(data.result == "false"){
				...
				$("#tab2_1").text("该帐号可以使用").removeClass('red').addClass('green').next("div.errorImg").addClass("trueImg").show();
			}
		});
	}
}
