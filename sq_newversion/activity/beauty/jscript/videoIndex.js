...
var cityInit={
		opIt:"<option value='none'>请选择</option>",
		init:function(prio,city){
				var seleObj = _dom.getElementById(prio);
				var cityObj = _dom.getElementById(city);
				seleObj.innerHTML = cityInit.optionIt(dsy.Items[0]);
				cityObj.innerHTML = cityInit.opIt;
				seleObj.onchange = function(){
						var idx = this.selectedIndex;
						cityObj.innerHTML = cityInit.optionIt(dsy.Items["0_" + (idx - 1)],1);
					}
			},
		optionIt:function(arr,t){
				if(arr == undefined) return cityInit.opIt;
				var prov = arr, provLen = prov.length;
				var opti = t ? "" : cityInit.opIt;
				for (var i = 0; i < provLen; i++){
							opti += "<option value='"+prov[i]+"'>"+ prov[i] +"</option>"
					}
				return opti;
			}
	}
cityInit.init("province","city");
...
/*
var setImage={
		imgType:["jpeg","jpg","bmp","gif","png"],//图片格式
		docObj:_dom.getElementById("fileD"),//input file对象
		tarObj:_dom.getElementById("localImag"),//目标对象
		mW:72,//默认宽度
		mH:64,//默认高度
		numPic:0,//图片的数量
		colBoo:true,//是否可以删除
		//验证图片格式
		valiType:function(obj){
					if (!RegExp("\.(" + setImage.imgType.join("|") + ")$", "i").test(obj.value.toLowerCase()))
						{
							obj.value="";
		...
		//添加图片
		addPre:function(obj){
				 if(setImage.numPic==3){ alert("最多只能添加三张照片!");return;}
				 if(!setImage.valiType(obj)){return};
				 setImage.imgObj="";  
				 if(setImage.docObj.files && setImage.docObj.files[0]){
						setImage.imgObj=_dom.createElement("img");              
      					setImage.imgObj.src = window.URL.createObjectURL(setImage.docObj.files[0]);
               		 }else{//IE
						 	setImage.docObj.select();
							var imgSrc=_dom.selection.createRange().text;
							//IE6
							if(navigator.userAgent.indexOf("MSIE 6.0")>0){
									setImage.imgObj=_dom.createElement("img");  
									setImage.imgObj.src=imgSrc;
								}else{
										try{
												setImage.imgObj=_dom.createElement("em");
												setImage.imgObj.className="imgIE7";
												setImage.addDele(setImage.imgObj);
												setImage.imgObj.style.width=setImage.mW+"px";
												setImage.imgObj.style.height=setImage.mH+"px";
												setImage.imgObj.style.filter="progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale)";
												setImage.imgObj.filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src = imgSrc;
											}catch(e){
													alert("您上传的图片格式不正确，请重新选择!");
													return ;
													}
							...
							_dom.selection.empty();
						 }
				//加载完后重置大小比例
				var loadEvt=(setImage.imgObj.onreadystatechange===undefined)? "onload" :"onreadystatechange";
				setImage.imgObj[loadEvt]=function(){
						if(this.readyState == "complete" || this.complete == true){
							setImage.reSize(this);
							}
					}
			},
		//重置图片大小
		reSize:function(obj){
				var scH=setImage.mH/obj.height;
				var scW=setImage.mW/obj.width;
				var sc=(scW<scH)? (obj.style.height=obj.height*scW+"px",obj.style.width=setImage.mW+"px")  : (obj.style.width=obj.width*scH+"px",obj.style.height=setImage.mH+"px");
				//添加删除功能
				setImage.addDele(obj);
			},
		addDele:function(obj){
				if(setImage.colBoo==true){
					var span=_dom.createElement("span");
					span.className="wrapImg";
					span.style.width=obj.style.width;
					span.style.height=obj.style.height;
					var aBt=_dom.createElement("a");
					aBt.className="deletBt";
					span.appendChild(obj);
					span.appendChild(aBt);
					//删除已添加的图片
					aBt.onclick=function(){
						var thPar=this.parentNode;
						setImage.tarObj.removeChild(thPar);
						setImage.numPic--;
						}
					}
				var obj=span || obj;
				setImage.tarObj.appendChild(obj);
				setImage.numPic++;
			}
	}
*/