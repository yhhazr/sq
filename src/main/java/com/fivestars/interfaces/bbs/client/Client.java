...
public class Client extends PHPFunctions{
	...
	public static String UC_IP = "localhost";
	...
	static {
	    InputStream in = Client.class.getClassLoader().getResourceAsStream("discuzConfig.properties");
...
//		}
	...
	protected String uc_fopen(String $url, int $limit, String $post, String $cookie, boolean $bysocket ,String $ip , int $timeout , boolean $block ) {
		...
		try{
			...
			$fp.setSoTimeout(3000);			
			if(!$fp.isConnected()) {
				return "";//note $errstr : $errno \r\n
			} else {
				logger.info("====bbs socket send start...");
				OutputStream os = $fp.getOutputStream();
				os.write($out.toString().getBytes());
				logger.info("====bbs socket send end...");
				logger.info("====bbs socket get 1 start...");
				...
				while (true) {
					...
					if($header == null || "".equals($header) || $header == "\r\n" || $header == "\n") {
						break;
					}
				}
				logger.info("====bbs socket get 1 end...");
				logger.info("====bbs socket get 2 start...");
				while (true) {
				...
				logger.info("====bbs socket get end...");
	...
	*/
	...
	public String uc_user_login(String $username, String $password, int $isuid , int $checkques , String $questionid, String $answer) {
		...
		args.put("questionid", $questionid);
		args.put("answer",$answer);
		String $return = call_user_func(UC_API_FUNC, "user", "loginToUc", args);
		return UC_CONNECT .equals("mysql") ? $return : uc_unserialize($return);
	}
