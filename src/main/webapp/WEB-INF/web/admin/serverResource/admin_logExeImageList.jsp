	...
	<tr>
		<td width="7%">			
				<s:submit key="admin.common.deleteChecked" 
					name="Submit" cssClass="button" theme="simple" onclick="deleteCheckedImage()"/>
			
		</td>
		<td align="left" width="93%">
			
				<s:submit key="admin.homepage.createImage"
					name="Submit" cssClass="button" theme="simple" onclick="createImage()"/>
			
		</td>
	</tr>
	<tr>
		<td colspan="2">
		<pg:pager items="<%=pageationResult.getTotal()%>" url='<%=request.getContextPath() + "/logexeImage/queryLogexeImageList.action"%>' index="center" maxPageItems='<%= Integer.parseInt(SystemConfig.getProperty("admin.search.page.size")) %>'
			...
			<display:table name="requestScope.pageationResult.resultList" class="tablelist" 
				...
				requestURI="" id="imageItems">
				<%-- Table columns --%>
				<display:column  title="<input type='checkbox' name='checklist'  onclick='checkAll()'>"> 
					<input type="checkbox" id="<s:property value='#attr.imageItems.id'/>">
				</display:column> 
				...
				<display:column  titleKey="admin.common.operation" href='<%=contextPath + "/logexeImage/editLogexeImage.action"%>'>
				<s:url id="editUrl" namespace="/logexeImage" action="editLogexeImage">
              		<s:param name="id"><s:property value="#attr.imageItems.id"/></s:param>
       			</s:url>
       			<s:a href="%{editUrl}"><s:text name="admin.common.edit"/></s:a> 
				<a href="#" onClick="deleteImage('<s:property value="#attr.imageItems.id"/>','<s:property value="#attr.imageItems.content"/>')"><s:text name="admin.common.delete"/></a>&nbsp;
				</display:column>
			</display:table>
...
<script type="text/javascript">
	highlightTableRows("imageItems");  

	function deleteImage(imageId,content){
		if(window.confirm("<s:text name="admin.serverResource.delete.message"><s:param>"+ content + "</s:param></s:text>"))
			location.href="<%=contextPath%>/logexeImage/removeLogexeImage.action?id="+imageId;
		else
			return;
	}
	function checkAll(){
	...
	function deleteCheckedImage(){
		...
		if(window.confirm("<s:text name="admin.common.deleteChecked.message"></s:text>"))
			location.href="<%=contextPath%>/logexeImage/removeLogexeImage.action?id="+ids;
		else
			return;		
	}
	function createImage(){
		location.href="<%=contextPath%>/logexeImage/createLogexeImage.action";
	}
