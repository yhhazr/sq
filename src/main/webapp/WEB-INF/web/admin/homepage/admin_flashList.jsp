...
<table style="margin-top: 10px; width: 100%" >
	...
	<tr>
		<td colspan="2">
		<pg:pager items="<%=pageationResult.getTotal()%>" url='<%=request.getContextPath() + "/homepageManage/queryFlashList.action"%>' index="center" maxPageItems='<%= Integer.parseInt(SystemConfig.getProperty("admin.search.page.size")) %>'
			...
			<display:table name="requestScope.pageationResult.resultList" class="tablelist" 
				...
				<s:url id="editUrl" namespace="/homepageManage" action="editFlash">
...
<script type="text/javascript">
	...
	function deleteItem(id,name){
		if(window.confirm("<s:text name="admin.item.delete.message"><s:param>"+ name + "</s:param></s:text>"))
			location.href="<%=contextPath%>/homepageManage/deleteFlash.action?id="+id;
		else
			return;
	}
	function checkAll(){
	...
	function deleteCheckedPhotoCat1(){
		...
		if(window.confirm("<s:text name="admin.common.deleteChecked.message"></s:text>"))
			location.href="<%=contextPath%>/homepageManage/deleteFlash.action?id="+ids;
		else
			return;		
	}
	function createItem(){
		location.href="<%=contextPath%>/homepageManage/createFlash.action";
	}
</script>	