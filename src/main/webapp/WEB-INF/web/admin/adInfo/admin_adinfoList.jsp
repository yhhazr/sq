...
<%
	...
	PaginationResult<AdInfo> paginationResult = (PaginationResult<AdInfo>)request.getAttribute("paginationResult");
...
<table style="margin-top: 10px; width: 100%" >
	<tr>
		<td colspan="2">
		<pg:pager items="<%=paginationResult.getTotal()%>" url='<%=request.getContextPath() + "/adInfoManage/queryAllAdinfo.action"%>' index="center" maxPageItems='<%= Integer.parseInt(SystemConfig.getProperty("admin.search.adinfo.page.size")) %>'
			...
			<display:table name="requestScope.paginationResult.resultList" class="tablelist" 
				...
				requestURI="" id="adinfo">
				<%-- Table columns --%>
				<display:column property="id"   titleKey="admin.id"/>
				<display:column property="adId"   titleKey="admin.ad.id"/>
				<display:column property="site"   titleKey="admin.ad.site"/>
				<display:column property="enterTimes"   titleKey="admin.ad.enter"/>
				<display:column property="regTimes"   titleKey="admin.ad.reg" />
				<display:column property="date"   titleKey="admin.ad.date" />
			</display:table>
